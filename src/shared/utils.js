...
function displayMongoshCommands(authMethod, connectionInfo) {
  ...
  switch (authMethod) {
    case 'password':
      ...
      console.log(`mongosh --host ${connectionInfo.host} --port ${connectionInfo.port} --username ${connectionInfo.username} --password --authenticationDatabase ${connectionInfo.authSource}`);
      break;
